...
(function (global, factory) {
	...
	*/
	...
	/**
	 * Navbar narrow functionality
	 * Adds tooltips to narrow sidebar navigation links
	 */

	const initNavbarNarrow = () => {
	  const narrowNavbar = document.querySelector('.navbar-narrow');
	  if (!narrowNavbar) {
	    return;
	  }

	  // Initialize tooltips for narrow sidebar nav links
	  const navLinks = narrowNavbar.querySelectorAll('.nav-link');
	  ...
	  if (typeof bootstrap !== 'undefined' && bootstrap.Tooltip) {
	    const tooltipTriggerList = narrowNavbar.querySelectorAll('[data-bs-toggle="tooltip"]');
	    tooltipTriggerList.forEach(tooltipTriggerEl => {
	      new bootstrap.Tooltip(tooltipTriggerEl, {
	        boundary: 'viewport',
	        customClass: 'navbar-narrow-tooltip'
	      });
	    });
	  }

	  // Handle dropdown positioning for narrow sidebar
	  const dropdowns = narrowNavbar.querySelectorAll('.dropdown');
	...
	document.addEventListener('DOMContentLoaded', initNavbarNarrow);
	      ...
	      if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
	        mutation.addedNodes.forEach(node => {
	          if (node.nodeType === Node.ELEMENT_NODE) {
	            if (node.classList?.contains('navbar-narrow') || node.querySelector?.('.navbar-narrow')) {
	              setTimeout(initNavbarNarrow, 100);
	            }
	          }
	        });
	      }
