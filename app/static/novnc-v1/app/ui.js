...
const UI = {
    ...
    notifyParent(kind, payload) {
        try {
            if (window.parent && window.parent !== window) {
                window.parent.postMessage({
                    source: "appcenter-novnc",
                    kind: kind,
                    payload: payload || {},
                }, "*");
            }
 ...
 * ------v------*/
    ...
    updateVisualState(state) {
        UI.notifyParent("state", { state: state });
    ...
    showStatus(text, statusType, time) {
        UI.notifyParent("status", { text: text || "", statusType: statusType || "normal" });
 ...
 * ------v------*/
    ...
    connect(event, password) {
        ...
        url += '/' + path;
        const token = WebUtil.getConfigVar('token', null);
        if (token) {
